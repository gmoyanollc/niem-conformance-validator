...
<stylesheet xmlns:catalog="urn:oasis:names:tc:entity:xmlns:xml:catalog"
  ...
  <function name="impl:get-clark-name" as="xs:string">
    ...
    <value-of
      select="concat('{', namespace-uri-from-QName($qname), '}', local-name-from-QName($qname))"/>
  </function>
  ...
  <function name="impl:get-target-namespace" as="xs:anyURI?">
    ...
    <choose>
      ...
      <otherwise>
        ...
        <sequence
          select="
            xs:anyURI(if (exists($target-namespace-attr)) then
              string($target-namespace-attr)
            else
              '')"
        />
      </otherwise>
  ...
  <function name="impl:resolve-namespace" as="element(xs:schema)?">
    ...
    <variable name="context-target-namespace-uri" as="xs:anyURI?"
      select="impl:get-target-namespace($context)"/>
    <choose>
      <!-- this SHOULD work for target namespace = xs:anyURI('') -->
      <when
        test="exists($context-target-namespace-uri) and exactly-one($context-target-namespace-uri) = $namespace-uri">
        <sequence select="root($context)/xs:schema"/>
      </when>
      <when test="empty($xml-catalog)">
        <sequence
          select="error(xs:QName('impl:xml-catalog-not-set'), 'Error: $xml-catalog is not set')"/>
      </when>
      <otherwise>
        <variable name="catalog-element" as="element(catalog:catalog)?"
          select="$xml-catalog/catalog:catalog"/>
        <choose>
  ...
  <function name="impl:resolve-type" as="element()?">
    ...
    <variable name="schema" as="element(xs:schema)?"
      select="impl:resolve-namespace($context, $namespace-uri)"/>
    <choose>
      <when test="empty($schema)">
        <sequence select="()"/>
      </when>
      <otherwise>
        <variable name="type" as="element()?"
          select="$schema/*[(self::xs:complexType or self::xs:simpleType) and @name = local-name-from-QName($qname)][1]"/>
        <choose>
  ...
  <function name="impl:has-effective-conformance-target-identifier" as="xs:boolean">
    ...
    <sequence
      select="
        if (empty($effective-conformance-targets-attribute)) then
          false()
        else
          some $effective-conformance-target-string in tokenize(normalize-space(string($effective-conformance-targets-attribute)), ' ')
            satisfies ($effective-conformance-target-string castable as xs:anyURI and xs:anyURI($effective-conformance-target-string) = $match)"
    />
  </function>
  ...
  <template match="catalog:nextCatalog" mode="impl:resolve-namespace" as="element(xs:schema)?">
    ...
    <choose>
      ...
      <otherwise>
        <variable name="catalog" as="element(catalog:catalog)?"
          select="doc($catalog-uri)/catalog:catalog"/>
        <choose>
          <when test="empty($catalog)">
            <message>
              <value-of select="impl:get-location(.)"/>
              <text>: catalog:nextCatalog/@catalog resolves to something that is not a
                catalog.</text>
            </message>
            <sequence select="()"/>
          </when>
